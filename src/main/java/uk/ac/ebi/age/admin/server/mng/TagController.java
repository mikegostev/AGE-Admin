...
public class TagController implements RemoteRequestListener
{
 ...
 public TagController(Configuration conf)
 {
  config = conf;
 }
 ...
 public boolean processRequest(ServiceRequest upReq, final PrintWriter out)
 {
  ...
  if(Configuration.getDefaultConfiguration().getPermissionManager().checkSystemPermission(SystemAction.CTRLSUBMTAGS, userName) != Permit.ALLOW)
  {
   out.println("ERROR: user '" + userName + "' is not allowed to manage submission tags");
   return false;
  }

  String val = upReq.getParams().get(SubmissionConstants.SUBMISSON_ID);

  if(val == null)
  {
   out.println("ERROR: No submission ID specified");
   return false;
  }
