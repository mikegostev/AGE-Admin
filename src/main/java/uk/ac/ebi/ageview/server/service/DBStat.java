...
public class DBStat extends HttpServlet
{
 ...
 {
  AgeViewStat stat = AgeViewService.getInstance().getStatistics();
 ...
 private void sendStat( PrintWriter out, AgeViewStat stat )
 {
   ...
   if( stat.getTopics() != null )
   {
    ...
    for(Map.Entry<String, AgeViewStat> me : stat.getTopics().entrySet() )
    {
     if( ! first )
      out.print(",\n");
     else
