...
import uk.ac.ebi.ageview.shared.TagControlConstants;

public class AgeViewTagController implements RemoteRequestListener
{
 private final Configuration config;
 private final AgeAdmin admin;

 public AgeViewTagController(AgeAdmin adm)
 {
  admin = adm;
 ...
 public boolean processRequest(ServiceRequest upReq, final PrintWriter out)
 {
  ...
  if(val != null)
  {
   ...
   for(int i = 0; i < rootObjId.length; i++)
    rootObjId[i] = rootObjId[i].trim();
  }
  ...
  if(  addTags != null ||  delTags != null )
  {
   try
   {
    ...
    if(rootObjId != null)
     if( !updateObjectTags(trn, rootObjId, addTags, delTags, auxList, AgeViewConfigManager.ROOT_CLASS_NAME, out) )
      return false;
  ...
  if( "on".equalsIgnoreCase(upReq.getParams().get(TagControlConstants.LIST_TAGS)) )
  {
   try
   {
    ...
    if(rootObjId != null)
     if( !printObjectTags(rLock, rootObjId, AgeViewConfigManager.ROOT_CLASS_NAME, out) )
      return false;
