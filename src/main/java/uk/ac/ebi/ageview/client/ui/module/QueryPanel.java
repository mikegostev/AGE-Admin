...
//  bt.addClickHandler( new QueryAction() );
  ...
  f1.setFields(sp0, sp1,queryField, sp2, sp3, what);
...
//   act.execute();
 ...
 private class QueryAction implements  FormItemClickHandler, KeyPressHandler, ScheduledCommand // ClickHandler,
 {
  ...
  public void execute()
  {
   ...
   AgeViewGWTService.Util.getInstance().selectRootObjects(query, 
     searchAttribNames, searchAttribValues, 0, ResultPane.MAX_GROUPS_PER_PAGE, new AsyncCallback<Report>()
     {
      ...
      {
       resultCallback.showResult(resLst, (String) queryField.getValue(), searchAttribNames, searchAttribValues,1);
      }
     });
 ...
 public void linkClicked(String param)
 {
  ...
  AgeViewGWTService.Util.getInstance().selectRootObjects(query,
    searchAttribNames, searchAttribValues, (pNum-1)*ResultPane.MAX_GROUPS_PER_PAGE, ResultPane.MAX_GROUPS_PER_PAGE, new AsyncCallback<Report>()
    {
     ...
     public void onSuccess(Report resLst)
     {
      resultCallback.showResult(resLst, (String) queryField.getValue(), searchAttribNames, searchAttribValues,pageNum);
     }
    }); }
