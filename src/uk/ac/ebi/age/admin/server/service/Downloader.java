...
public class Downloader
{
  ...
  */
 public static void processRequest(HttpServletRequest request, HttpServletResponse response, boolean content, File file, String contentType)
   throws IOException
 {
  ...
  // If-Unmodified-Since header should be greater than LastModified. If not,
  // then return 412.
  ...
  if(range != null)
  {
   ...
   // Range header should match format "bytes=n-n,n-n,n-n...". If not, then
   // return 416.
   if(!range.matches("^bytes=\\d*-\\d*(,\\d*-\\d*)*$"))
   {
    response.setHeader("Content-Range", "bytes */" + length); // Required in
                                                              // 416.
   ...
   if(ifRange != null && !ifRange.equals(eTag))
   {
    try
    {
     long ifRangeTime = request.getDateHeader("If-Range"); // Throws IAE if
                                                           // invalid.
   ...
   if(ranges.isEmpty())
   {
    ...
    {
     // Assuming a file with length of 100, the following examples returns bytes
     // at:
     // 50-80 (50 to 80), 40- (40 to length=100), -20 (length-20=80 to
     // length=100).
 ...
 private static void close(Closeable resource)
 {
  if(resource != null)
  {
   ...
   {
    // Ignore IOException. If you want to handle this anyway, it might be useful
    // to know
    // that this will generally only be thrown when the client aborted the
    // request.
   }
  }
 }
